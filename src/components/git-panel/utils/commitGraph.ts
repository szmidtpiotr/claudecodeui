...
/** @deprecated Use CommitGraphRow */
export type GraphRow = CommitGraphRow & {
  lane: number;
  color: string;
...
/** @deprecated Use computeCommitGraph */
export function computeGraphRows(commits: GraphCommit[]): GraphRow[] {
  return computeCommitGraph(commits).map((row) => ({
    ...row,
    lane: row.nodeLane,
    color: laneColor(row.nodeLane),
    edges: row.passThrough.map((lane) => ({ fromLane: lane, toLane: lane, color: laneColor(lane) })),
    mergeEdges: row.outbound.map((lane) => ({ fromLane: row.nodeLane, toLane: lane, color: laneColor(lane) })),
  }));
}
